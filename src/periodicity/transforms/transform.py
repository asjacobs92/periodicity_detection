...
def dwt(signal, wavelet="haar", levels=2, decimated=True, pad=False):
    ...
    for level in range(1, levels + 1):
        ...
        scale = len(lp_filter)
        for start in range(
            0, len(lp_list[level - 1]), 2 if decimated else 1
        ):  # redundant dwt, no downsampling
            lp, hp = 0, 0
        ...
        if pad:
            lp_list[level] = list(
                np.pad(
                    lp_list[level],
                    (0, len(signal) - len(lp_list[level])),
                    "constant",
                    constant_values=(0, 0),
                )
            )
            hp_list[level] = list(
                np.pad(
                    hp_list[level],
                    (0, len(signal) - len(hp_list[level])),
                    "constant",
                    constant_values=(0, 0),
                )
            )
...
def dwt_haar(signal, levels=2, decimated=True, pad=False):
    """
    Applyies a discrete wavelet transform on the input signal.
    Args:
        - signal
    ...
    for level in range(1, levels + 1):
        ...
        for start in range(
            ...
            lp, hp = 0, 0
            # Applying filter to each point in the signal according to wavelet scale (convolution)
            if start < len(lp_list[level - 1]):
                point_one = lp_list[level - 1][(start - 1) % len(lp_list[level - 1])]
                point_two = lp_list[level - 1][(start - 2) % len(lp_list[level - 1])]
                lp = int((point_one + point_two) / 2)
                hp = int((point_one - point_two) / 2)
