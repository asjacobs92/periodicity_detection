...
from periodicity.transforms import dwt, swt, dwt_haar, dwt_int
from periodicity.utils import factory, plotter


def wavelet_test():
    """Tests wavelets implementation against library"""
    print("---------------- Start ----------------")
    signal, impulse, noise = factory.make_signal(3000, 100, 10, 0)
    plotter.plot_signal(signal, impulse, noise, "Signal")

    # signal = factory.make_cos_signal()

    [cA8, cD8, cD7, cD6, cD5, cD4, cD3, cD2, cD1] = pywt.wavedec(signal, "db1", level=8)
    ...
    hp_list, _ = dwt_haar(signal, levels=8)
    hp_list_norm, _ = dwt_int(signal, "db1", levels=8)

    # for i in range(1, len(hp_list)):
    # print("HP Length: ", i, len(hp_list[i]))
    # print("HP list normalized", i, len(hp_list_norm))
    # print("LP Length: ", i, len(lp_list[i]), len(lp_list_pywt[i]))

    # is_equal_hp = True
    # for d1, d2 in zip(hp_list[i], hp_list[i]):
    ...
    plotter.plot_coefficients(hp_list, "(1) Detail level Haar Wavelet")
    plotter.plot_variance_by_level(
        hp_list,
        "(1) Detail level Variance by Level",
    )

    plotter.plot_coefficients(hp_list_norm, "(2) Detail level Normalized Haar Wavelet")
    plotter.plot_variance_by_level(
        hp_list_norm,
        "(2) Detail level Normalized Haar Wavelet Variance by Level",
    )

    plotter.plot_coefficients(hp_list_pywt, "(3) Detail level PyWT Haar Wavelet")
    plotter.plot_variance_by_level(
        hp_list_pywt,
        "(3) Detail level PyWT Haar Wavelet Variance by Level",
    )
    ...
    with open("../res/ratio/results.csv", "w") as csv_file:
        ...
        for ratio in range(1, 11):
            for index in range(30):
                # generate signal
                signal, impulse, noise = factory.make_signal(
                    impulse_amp, impulse_fq, impulse_amp * ratio
                )
                ...
                for level, coeffs in dwt_hp_list.items():
                    pdm_scanner = py_pdm.Scanner(
                        minVal=10, maxVal=100, dVal=1, mode="period"
                    )
                    if len(coeffs) >= 10:
                        pdm = py_pdm.PyPDM(
                            np.asarray(range(len(coeffs))), np.asarray(coeffs)
                        )
                ...
                for level, coeffs in swt_hp_list.items():
                    pdm_scanner = py_pdm.Scanner(
                        minVal=10, maxVal=100, dVal=bin_size, mode="period"
                    )
                    if len(coeffs) >= 10:
                        pdm = py_pdm.PyPDM(
                            np.asarray(range(len(coeffs))), np.asarray(coeffs)
            ...
            plotter.plot_signal(
                ...
                impulse,
                noise,
                filename="../res/ratio/signal_{}_{}_{}.png".format(
                    impulse_amp, impulse_fq, impulse_amp * ratio
                ),
            )
            ...
            plotter.plot_coefficients(
                dwt_hp_list,
                filename="../res/ratio/dwt/coeffs_hp_ratio_{}.png".format(ratio),
            )
            plotter.plot_coefficients(
                dwt_lp_list,
                filename="../res/ratio/dwt/coeffs_lp_ratio_{}.png".format(ratio),
            )
            plotter.plot_coefficients(
                swt_hp_list,
                filename="../res/ratio/swt/coeffs_hp_ratio_{}.png".format(ratio),
            )
            plotter.plot_coefficients(
                swt_lp_list,
                filename="../res/ratio/swt/coeffs_lp_ratio_{}.png".format(ratio),
            )

            plotter.plot_pdm_by_level(
                dwt_hp_pdm_by_level,
                filename="../res/ratio/pdm/dwt_hp_pdm_ratio_{}.png".format(ratio),
            )
            plotter.plot_pdm_by_level(
                dwt_lp_pdm_by_level,
                filename="../res/ratio/pdm/dwt_lp_pdm_ratio_{}.png".format(ratio),
            )

            plotter.plot_pdm_by_level(
                swt_hp_pdm_by_level,
                filename="../res/ratio/pdm/swt_hp_pdm_ratio_{}.png".format(ratio),
            )
            plotter.plot_pdm_by_level(
                swt_lp_pdm_by_level,
                filename="../res/ratio/pdm/swt_lp_pdm_ratio_{}.png".format(ratio),
            )
    ...
    with open("../res/frequency/results.csv", "w") as csv_file:
        ...
        writer.writerow(
            [
                ...
                "detected periods",
                "theta",
                "num detected impulses",
            ]
        )

        for index in range(30):
            # generate signal
            signal, impulses, noise = factory.make_complex_signal(
                impulses_info, noise_amp
            )
            plotter.plot_multiple_signals(
                signal, impulses, noise, filename="../res/frequency/signal_multiple.png"
            )
            ...
            plotter.plot_coefficients(
                hp_list, filename="../res/frequency/coeffs_hp_ratio.png"
            )
            plotter.plot_coefficients(
                lp_list, filename="../res/frequency/coeffs_lp_ratio.png"
            )
            ...
            for level, coeffs in lp_list.items():
                pdm_scanner = py_pdm.Scanner(
                    minVal=10, maxVal=300, dVal=1, mode="period"
                )
                if len(coeffs) >= 10:
                    pdm = py_pdm.PyPDM(
                        np.asarray(range(len(coeffs))), np.asarray(coeffs)
                    ...
                    writer.writerow(
                        [
                            index,
                            [fq for (amp, fq) in impulses_info],
                            level,
                            "lp",
                            detected_periods,
                            detected_periods_thetas,
            ...
            plotter.plot_pdm_by_level(
                lp_pdm_by_level, filename="../res/frequency/lp_pdm_.png"
            )
            plotter.plot_pdm_by_level(
                hp_pdm_by_level, filename="../res/frequency/hp_pdm_.png"
            )
...
def int_error_test():
    ...
    # signal = factory.make_cos_signal()
    signal, impulse, noise = factory.make_signal(
        impulse_amp, impulse_fq, impulse_amp * 2
    )
    ...
    for i in range(1, min(len(hp_list), len(hp_list_int))):
        ...
        print(
            "MSE of HP level {}:".format(i),
            mean_squared_error(hp_list[i], hp_list_int[i]),
        )
        print(
            "MAE of HP level {}:".format(i),
            mean_absolute_error(hp_list[i], hp_list_int[i]),
        )
        ...
        print(
            "MSE of LP level {}:".format(i),
            mean_squared_error(lp_list[i], lp_list_int[i]),
        )
        print(
            "MAE of LP level {}:".format(i),
            mean_absolute_error(lp_list[i], lp_list_int[i]),
        )
    ...
    if pad:
        print(
            "Total MSE of HP:",
            mean_squared_error(list(hp_list.values()), list(hp_list_int.values())),
        )
        print(
            "Total MAE of HP:",
            mean_absolute_error(list(hp_list.values()), list(hp_list_int.values())),
        )
        print(
            "Total R2 of HP:",
            r2_score(list(hp_list.values()), list(hp_list_int.values())),
        )

        print(
            "Total MSE of LP:",
            mean_squared_error(list(lp_list.values()), list(lp_list_int.values())),
        )
        print(
            "Total MAE of LP:",
            mean_absolute_error(list(lp_list.values()), list(lp_list_int.values())),
        )
        print(
            "Total R2 of LP:",
            r2_score(list(lp_list.values()), list(lp_list_int.values())),
        )
